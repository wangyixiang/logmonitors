...
- 平台与语言
  ...
  - 首选语言：C#（.NET 8），无管理员权限即可运行。
  - 备选实现：C++ Win32（如遇到 .NET API 局限，请同时提供 C++ 版本）。
...
- 日志落盘
  - 写入 `C:\ProgramData\MonitorEvents\events.log`（若无则自动创建目录）。
...
- 运行形态
  - 提供两种形态：
    - 托盘应用（带最小 UI）：启动/停止、实时事件计数、日志位置打开。
...
- 交付物
  - 完整源码（C# .NET 8），包含：
    - 事件监听（窗口类 + `RegisterDeviceNotification`）、WMI 订阅、`QueryDisplayConfig`/DXGI 枚举。
    - 日志模块（JSON Lines + 滚动）。
    - 简单托盘 UI（可隐藏运行）。
    - 结构清晰的项目（`Program.cs`、事件采集器、枚举器、日志器、UI）。
  - 清晰的构建与运行说明（无需管理员），以及如何切换到无界面模式。
...
请直接给出：
1) 完整实现代码（优先 C# .NET 8，若需补充 C++ Win32 可附上），
2) 构建与运行步骤，
